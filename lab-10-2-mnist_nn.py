...
def network(x, reuse=False) :
    with tf.variable_scope('network', reuse=reuse) :
        ...
        for i in range(2) :
            x = tf.layers.dense(inputs=x, units=256, use_bias=True, name='fully_connected_' + str(i))
            x = tf.nn.relu(x)

        hypothesis = tf.layers.dense(inputs=x, units=10, use_bias=True, name='fully_connected_logit')
